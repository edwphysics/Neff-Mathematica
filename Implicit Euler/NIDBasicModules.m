...
f\[Nu]0[\[Alpha]_]:=1/(1.+Exp[\[Alpha]/1.00003]);


(* ::Input::Initialization:: *)
(*Non-dimensional Hubble Parameter*)
H[f\[Nu]e_,f\[Nu]\[Mu]_,z_Real,x_Real]:=H[f\[Nu]e,f\[Nu]\[Mu],z,x]=(me/x)^2 Sqrt[(8 N[\[Pi]])/(3.((Mpl)^2) ) \[Rho]Bar[f\[Nu]e,f\[Nu]\[Mu],z,x]];
(*Total Energy Density*)
\[Rho]Bar[f\[Nu]e_,f\[Nu]\[Mu]_,z_Real,x_Real]:=\[Rho]Bar[f\[Nu]e,f\[Nu]\[Mu],z,x]=(N[\[Pi]]^2 z^4)/15.+NIntegrate[2./N[\[Pi]]^2 (y^2 Sqrt[y^2+x^2])/(Exp[1/z Sqrt[y^2+x^2]]+1)+1./N[\[Pi]]^2 y^3 (f\[Nu]e[y]+2.f\[Nu]\[Mu][y]),{y,ymin,ymax},Method->{Automatic,"SymbolicProcessing"->0}];
...
E2P5:=eE4+eE3-y;
y2P1:=E2P1;
y2P4:=Sqrt[E2P4^2-x^2]UnitStep[E2P4^2-x^2];
y2P5:=E2P5;
...
\[CapitalDelta]f\[Nu]e[F\[Nu]e_,F\[Nu]\[Mu]_,Z_Real,X_Real,Y_Real]:=1./(X H[F\[Nu]e,F\[Nu]\[Mu],Z,X]) GF^2/(2.N[\[Pi]]^3) 1/Y (me/X)^4 eIntegral[F\[Nu]e,F\[Nu]\[Mu],Z,X,Y]
...
\[CapitalDelta]f\[Nu]\[Mu][F\[Nu]e_,F\[Nu]\[Mu]_,Z_Real,X_Real,Y_Real]:=1./(X H[F\[Nu]e,F\[Nu]\[Mu],Z,X]) GF^2/(2.N[\[Pi]]^3) 1/Y (me/X)^4 \[Mu]Integral[F\[Nu]e,F\[Nu]\[Mu],Z,X,Y]


(* ::Input::Initialization:: *)
(*Internal Integrals*)
F1[\[Mu]_]:=NIntegrate[\[Omega]^2 Exp[Sqrt[\[Omega]^2+\[Mu]^2]]/(Exp[Sqrt[\[Omega]^2+\[Mu]^2]]+1.)^2,{\[Omega],ymin,ymax}];
F2[\[Tau]_]:=NIntegrate[\[Omega]^4 Exp[Sqrt[\[Omega]^2+\[Tau]^2]]/(Exp[Sqrt[\[Omega]^2+\[Tau]^2]]+1.)^2,{\[Omega],ymin,ymax}];


(* ::Input::Initialization:: *)
CE[df\[Nu]e_,df\[Nu]\[Mu]_,z_Real,x_Real]:=1/((x/z)^2 F1[x/z]+F2[x/z]+((2N[\[Pi]]^4)/15.)) (x/z F1[x/z]-1/(2.z^3) NIntegrate[y^3 (df\[Nu]e[y]+2.df\[Nu]\[Mu][y]),{y,ymin,ymax},AccuracyGoal->10]);
...
(*Spectra Distorions*)
distortionsf\[Nu]e={{xin,1.}};
distortionsf\[Nu]\[Mu]={{xin,1.}};
